...
class QA_MultiQA(QA_baseline):
    ...
    def score_time(self, head_embedding, tail_embedding, relation_embedding):
        ...
        return (
                (0.7 * lhs[0] * rel[0] * rhs[0] - 0.3 * lhs[1] * rel[1] * rhs[0] -
                 0.7 * lhs[1] * rel[0] * rhs[1] + 0.3 * lhs[0] * rel[1] * rhs[1]) @ time[0].t() +
                (0.7 * lhs[1] * rel[0] * rhs[0] - 0.3 * lhs[0] * rel[1] * rhs[0] +
                 0.7 * lhs[0] * rel[0] * rhs[1] - 0.3 * lhs[1] * rel[1] * rhs[1]) @ time[1].t()
        )
    ...
    def score_entity(self, head_embedding, tail_embedding, relation_embedding, time_embedding):
        ...
        return (
                (0.7 * lhs[0] * full_rel[0] - 0.3 * lhs[1] * full_rel[1]) @ right[0].t() +
                (0.7 * lhs[1] * full_rel[0] + 0.3 * lhs[0] * full_rel[1]) @ right[1].t()
        )
    ...
    def forward(self, a):
        ...
        scores_entity = 0.6 * scores_entity1 + 0.4 * scores_entity2
        scores = torch.cat((scores_entity, scores_time), dim=1)
        return scores
