...
import pickle
import numpy as np
from qa_baselines import QA_baseline, QA_lm, QA_embedkgqa, QA_cronkgqa, QA_MultiQA, QA_TKGSQ_PM
from qa_datasets import QA_Dataset_cron, QA_Dataset_TKGSQ_PM, QA_Dataset_Baseline_cron, QA_Dataset_Baseline_TKGSQ_PM, QA_Dataset_TKGSQ_PM_semantic
from torch.utils.data import Dataset, DataLoader
import utils
from tqdm import tqdm
from utils import loadTkbcModel, loadTkbcModel_complex, print_info, save_model, append_log_to_file, train, eval_cron,eval_multi,eval_TKGSQ_PM
...
if args.dataset_name == 'CronQuestions':
    QA_Dataset_Baseline = QA_Dataset_Baseline_TKGSQ_PM
    QA_Dataset_TKGSQ_PM = QA_Dataset_TKGSQ_PM_semantic
    eval = eval_TKGSQ_PM
elif args.dataset_name == 'timequestions':
    QA_Dataset_Baseline = QA_Dataset_Baseline_TKGSQ_PM
    QA_Dataset_TKGSQ_PM = QA_Dataset_TKGSQ_PM_semantic
    eval = eval_TKGSQ_PM
...
elif args.model == 'TKGSQ_PM':
    dataset = QA_Dataset_TKGSQ_PM(split=train_split, dataset_name=args.dataset_name, args=args)
    valid_dataset = QA_Dataset_TKGSQ_PM(split=args.eval_split,dataset_name=args.dataset_name,args = args)
    test_dataset = QA_Dataset_TKGSQ_PM(split=test, dataset_name=args.dataset_name, args=args)
    qa_model = QA_TKGSQ_PM(tkbc_model, args)
